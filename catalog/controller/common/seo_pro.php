	...
	public function __construct($registry) {
		...
		if (!$this->cache_data) {
			...
			foreach ($query->rows as $row) {
                ...
                // 301 Redirect from old SEF
                $redirect_queries = $this->db->query("SELECT `keyword`, `query` FROM " . DB_PREFIX . "url_redirect WHERE `query` = '" . $this->db->escape($row['query']) . "'");
                if ($redirect_queries->num_rows) {
                    foreach ($redirect_queries->rows as $redirect) {
                        $this->cache_data['keywords'][$redirect['keyword']] = $redirect['query'];
                        $this->cache_data['queries'][$redirect['query']] = $redirect['keyword'];
                    }
                }
